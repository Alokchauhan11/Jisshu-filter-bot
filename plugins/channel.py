...
async def send_movie_updates(bot, file_name, caption, file_id, message):
    try:
        ...
        # DEBUG: Print out whether IMDb poster is available
        print(f"IMDb poster found: {bool(poster_url)}")
        ...
        if not poster_url:
            if message.document and message.document.thumb:
                ...
                print("No thumbnail found in document or video.")
        ...
        if poster_url:
            print("Sending photo with poster.")
            ...
            print("Sending photo with fallback poster.")
            await bot.send_photo(
                movie_update_channel if movie_update_channel else MOVIE_UPDATE_CHANNEL,
                photo=no_poster,
                caption=caption_message,
